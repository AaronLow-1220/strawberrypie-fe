...
const SocialMediaIcon = memo(({ src, alt }) => (
  <div className="bg-[#6C2028] w-[40px] lg:w-[48px] lg:h-[48px] h-[40px] rounded-[50%] flex items-center justify-center transition-all duration-500 hover:bg-primary-color">
    <img className="w-[20px] lg:w-[24px] lg:h-[24px] h-[20px]" src={src} alt={alt} />
  </div>
));
...
export const FocusCard = memo(({
  ...
  const WideScreenLayout = (
    <div className="w-full h-full flex justify-center px-16" onClick={handleContainerClick}>
      <div className="my-auto">
        <div className="focus-card my-8 max-w-[1200px] bg-[#361014] p-[64px] rounded-[48px] flex gap-[48px] relative">
          <div className="w-full">
            <img className="aspect-[4/3] bg-white w-full rounded-[8px]" src={img} alt={title || "卡片圖片"} />
            <div className="px-3 mt-5">
              <div className="text-[20px] text-white opacity-[80%] mt-5">成員：</div>
              ...
              <div className="text-[20px] text-white opacity-[80%] mt-4">指導老師：</div>
              <div className="flex flex-wrap">
          ...
          <div className="relative flex flex-col justify-center align-top">
            <div className="w-full aspect-[4/3] relative rounded-t-[12px]">
              <button
                ...
                aria-label="關閉"
              >
                <img className="w-6 h-6" src="/Group/close.svg" alt="關閉按鈕" />
              </button>
              <img className="w-full h-full object-cover bg-white" src={img} alt={title || "卡片圖片"} />
            ...
            <div className="bg-[#361014] p-[20px_24px_32px_24px] flex flex-col flex-grow">
              ...
              <div className="text-[14px] text-white opacity-[80%] mt-5">成員：</div>
              <div className="flex flex-wrap">
                {member?.map((item, index) => (
                  <div key={index} className="text-[14px] mt-[2px] text-white">
                    {item}
                    {index !== member.length - 1 && "、"}
                  </div>
                ))}
              </div>
              <div className="text-[14px] text-white opacity-[80%] mt-3">指導老師：</div>
              <div className="flex flex-wrap">
                {teachers?.map((item, index) => (
                  <div key={index} className="text-[14px] mt-[2px] text-white">
                    {item}
