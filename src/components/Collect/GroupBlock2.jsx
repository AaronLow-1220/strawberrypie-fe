...
export const GroupBlock2 = ({ num }) => {
  ...
  const animateGridTransition = (expanding) => {
    ...
    items.forEach((item, i) => {
      ...
      const deltaX =
        startPos.left -
        (finalRect.left - newBodyRect.left + body.scrollLeft - paddingLeft);
      const deltaY = startPos.top - (finalRect.top - newBodyRect.top);

      // 計算圖標相對於項目的位置差異
      const iconDeltaX =
        startPos.iconLeft - (finalIconRect.left - finalRect.left);
      ...
      iconEl.style.transform = `translate(${iconDeltaX}px, ${iconDeltaY}px) scale(${
        scaleX / wrapperScaleX
      }, ${scaleY / wrapperScaleY})`;
  ...
  return (
    ...
    >
      <div className="flex justify-between p-[16px_20px_0px_20px] mb-4 items-center cursor-pointer" onClick={handleToggle}>
        ...
        <div
          className={`group-block__header__dropdown  ${
            isExpanded ? "expand" : ""
          }`}
        >
